    ...
    try {
      const { data, error } = await supabase
        ...
        .select(`
          ...
          date_added,
          expiry_date,
          teacher:teacher_id ( id, full_name, email, phone, city )
        `)
              ...
              <div className="space-y-3 mt-3">
                ...
                {myTeachers.map((m) => (
                  <div key={m.id} className="p-3 border rounded">
                    <div className="font-semibold">{m.teacher.full_name}</div>
                    <div className="text-sm">{m.teacher.email} | {m.teacher.phone}</div>
                    <div>Date added: {m.date_added} — Expires: {m.expiry_date}</div>
                  </div>
                ))}
              </div>
            </div>
